...
namespace Azure.ScannerEUI.ViewModel
{
    ...
    class Workspace : ViewModelBase, IDisposable
    {
        ...
        private Visibility _VesionVisFlag = Visibility.Visible;
        private bool _EnabledFlag = true;
        private bool _TopCoverLockColor;
        private bool _TopMagneticStateColor;
        private bool _OpticalModulePowerStatus;
        ...
        private ModuleInfoViewModel _ModuleInfoViewModel = null;
        ...
        public Workspace()
        {
            ...
            _ModuleInfoViewModel = new ModuleInfoViewModel(_EthernetController);
        ...
        public void ExecuteDisconnectDeviceCommand(object parameter)
        {
            ...
            {
                boxResult = MessageBox.Show("Close the optical module?", "Warning", MessageBoxButton.YesNo);
                if (boxResult == MessageBoxResult.Yes)
                {
                    ...
                    Workspace.This.IVVM.TurnOffAllLasers();//关闭所有激光器
                    _EthernetController.SetShutdown(1);  //下电
                }
            }
            else
            {
                boxResult = MessageBox.Show("Open the optical module?", "Warning", MessageBoxButton.YesNo);
                if (boxResult == MessageBoxResult.Yes)
                {
                 
                    _EthernetController.SetShutdown(0);  //上电
                }
            }
        }
        ...
        public void LoadInitSplashScreen()
        {
            try
            {
        ...
        public bool TopCoverLockColor
        {
            get { return _TopCoverLockColor; }
            set
            {
                if (_TopCoverLockColor != value)
                {
                    _TopCoverLockColor = value;
                    RaisePropertyChanged("TopCoverLockColor");
                }
            }
        }

        public bool TopMagneticStateColor
        {
            get { return _TopMagneticStateColor; }
            set
            {
                if (_TopMagneticStateColor != value)
                {
                    _TopMagneticStateColor = value;
                    RaisePropertyChanged("TopMagneticStateColor");
                }
            }
        }
        private bool OneLoadInitSplashScreen = false;
        private int OpticalModulePowerStatusCountTrue = 0;
        ...
        public bool OpticalModulePowerStatus
        {
            ...
            set
            {
                if (_OpticalModulePowerStatus != value)
                {
                    ...
                    {
                        OpticalModulePowerStatusCountTrue++;
                        if (!OneLoadInitSplashScreen)//软件第一次上电时
                        {
                            LoadInitSplashScreen();
                            OpticalModulePowerStatusCountTrue = 0;
                        }
                        else /*if (OpticalModulePowerStatusCountTrue == 1)*///只执行连续收到的第二次光学模块上电指令,避免两次发送电机Home指令。
                        {
                            OpticalModulePowerStatusCountTrue = 0;
                            NoShowLoadInit();
                            // return;
                        ...
                        //else if (OpticalModulePowerStatusCountTrue == 2)
                        //{
                        //    OpticalModulePowerStatusCountTrue = 0;
                        //    // LoadInitSplashScreen();
                        //    NoShowLoadInit();
                        ...
                        {
                            if (This.EthernetController.GetDeviceProperties() == false)
                            {
                            ...
                            ScanIsAlive = true;
                            This.EthernetController.SetIncrustationFan();//设置外壳风扇自动调整
                            Workspace.This.IVVM.VisbleAndEnable();
                            //Workspace.This.IVVM.IsCaptrueL1Selected = false;
                            //Workspace.This.IVVM.IsCaptrueR2Selected = false;
                            //Workspace.This.IVVM.IsCaptrueR1Selected = false;
                        }
                        else
                        {
        ...
        {
            get { return _ModuleInfoViewModel; }
        }
