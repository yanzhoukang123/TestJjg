...
#define MyAppVersion "1.2.0709.0"
...
Source: ..\Projects\Azure.LaserScanner\Build\Release\Azure.CameraLib.dll; DestDir: {app}; Flags: ignoreversion
Source: ..\Projects\Azure.LaserScanner\Build\Release\Azure.CommandLib.dll; DestDir: {app}; Flags: ignoreversion
Source: ..\Projects\Azure.LaserScanner\Build\Release\Azure.GalilMotor.dll; DestDir: {app}; Flags: ignoreversion
...
; Source: ..\Projects\Azure.LaserScanner\Build\Release\config.xml; DestDir: "{commonappdata}\Azure Biosystems\Sapphire"; Flags: ignoreversion
...
Source: ..\Projects\Azure.LaserScanner\Build\Release\Hats.APDCom.dll; DestDir: {app}; Flags: ignoreversion
Source: ..\Projects\Azure.LaserScanner\Build\Release\Method.xml; DestDir: {app}; Flags: ignoreversion
Source: ..\Projects\Azure.LaserScanner\Build\Release\PVCam.NET.dll; DestDir: {app}; Flags: ignoreversion
Source: ..\Projects\Azure.LaserScanner\Build\Release\Sapphire User Manual.pdf; DestDir: {app}; Flags: ignoreversion
...
var
  ...
  MethodXmlPath : string;
begin
  // Preserve old configuration files by renaming them with .old extension.
  RenameConfigXmlFile();
  ...
  if (FileExists(ExpandConstant('{commonappdata}\Azure Biosystems\Sapphire\Method.xml'))) then
  begin
    MethodXmlPath := ExpandConstant('{commonappdata}\Azure Biosystems\Sapphire\Method.xml')
  end
  else begin
    ExtractTemporaryFile('Method.xml');
    MethodXmlPath := ExpandConstant('{tmp}\Method.xml')
  end

  // Launch Azure.UpdateConfig.exe to update the 'config.xml' and 'SysSettings.xml' and wait for it to terminate
  execParam := InstallationType + ' ' + InstallationModule + ' ' + '"' + ConfigPath + '"' + ' ' + '"' + SysSettingsPath + '"'  + ' ' + '"' + MethodXmlPath + '"';
  ...
  begin
    ...
    result := FileCopy(ConfigPath, ExpandConstant('{app}\config.xml'), false);
    // copy modified 'SysSettings.xml' file from temp folder to app Program folder
    result := FileCopy(SysSettingsPath, ExpandConstant('{app}\SysSettings.xml'), false);
    // copy modified 'Method.xml' file from temp folder to app Program folder
    result := FileCopy(MethodXmlPath, ExpandConstant('{app}\Method.xml'), false);
    ...
    begin
      ...
      result := FileCopy(ConfigPath, ExpandConstant('{commonappdata}\Azure Biosystems\Sapphire\config.xml'), false);
    end
  end
  else begin
